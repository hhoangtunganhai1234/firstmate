...
family_for_basename() {
  case "$1" in
    ...
    fm-test-run.test.sh|fm-test-isolation-proof.test.sh)
      printf '%s\n' pure-contract-unit
      ;;
    fm-codex-telegram-waker.test.sh|fm-daemon.test.sh|fm-guard-stale-banner.test.sh|fm-pi-watch-extension.test.sh|\
    ...
    fm-domain-question-live-e2e.test.sh|\
...
tests/fm-domain-question-live-e2e.test.sh 19
tests/fm-codex-telegram-waker.test.sh 24181
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-codex-telegram-waker.sh|bin/fm-watch*|bin/fm-wake*|bin/fm-inactive-reconcile.sh|\
