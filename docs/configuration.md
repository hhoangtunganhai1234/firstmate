...
Each OS boundary exposes only the selected domain memory, its binding, and its bound data source read-only, with private scratch and answer directories as the only writable host surfaces.
